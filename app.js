...
function renderArtists() {
  ...
  Object.entries(artistsYearData || {}).forEach(([artist, data]) => {
...
function renderAlbums() {
  ...
  Object.values(window.albumsData || {}).forEach(alb => {
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td>${alb.album}</td>
      <td>${alb.artist}</td>
      <td>${(alb.sales || 0).toLocaleString()}</td>
      <td>${(alb.plays || 0).toLocaleString()}</td>
      <td>${alb.label || "N/A"}</td>
    `;
    tbody.appendChild(tr);
  });
